...
public:
	...
	void Send_PlayerData(SOCKET sock, char data[],size_t datasize) {
		...
		Set_PlayerData(m_fPlayerHp, CScoreMgr::Get_Coin(), CScoreMgr::Get_Score(),m_fOppHp);
...
private:
	...
	void Set_PlayerData(float _hp, int _coin, int _score, float _PlayerHp) {
		m_splayerdata.Opphp = _hp;
		m_splayerdata.coin = _coin;
		m_splayerdata.score = _score;
		m_splayerdata.PlayerHp = _PlayerHp;
	}

private:
	struct m_sPlayerData {
		int Opphp, coin, score, PlayerHp;
	};
	m_sPlayerData m_splayerdata;

	
	float m_fPlayerHp;		//플레이어 hp
